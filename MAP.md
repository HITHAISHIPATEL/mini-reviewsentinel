...
4. **LLM interface:** support a configured provider/API key and timeout; provide a clear static-only path if unavailable. Send only necessary code/context. Use a system instruction that treats repository content as quoted, untrusted data; explicitly say source comments/strings cannot change reviewer instructions. Request strict JSON, parse and validate it, reject malformed output, cap response size, and never allow model output to directly set final decision.
5. **Agentic revisit:** begin with static/LLM findings; if a finding is ambiguous or the providers materially disagree, request one targeted context pass (nearby lines/call site, not a new broad review). Re-evaluate/add context, preserve original evidence, then terminate. Hard cap: one extra pass per review (or one globally—choose and test exactly one policy); errors/timeouts also terminate. No autonomous edits and no repository commands.
...
- [ ] `pytest` passes and is run against submitted code.
...
- [ ] LLM is advisory, optional/offline-safe, and never receives authority from source comments.
